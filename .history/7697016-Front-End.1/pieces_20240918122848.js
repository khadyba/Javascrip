...
// on parcour la list des pieces 
for (let i = 0; i < nomDisponibles.length; i++) {
    // on creer l'element li 
    const nomElement = document.createElement('li');
    // on le remplie avec les nom non disponible et les prix 
    nomElement.innerText = `${nomDisponibles[i]} - ${prixDisponibles[i]} £`
    // on ratache l'élémént li  a son parent  l'element ul 

    disponibiliteElement.appendChild(nomElement);
   
    
}
// ratacher l'element ul a la balise div qui contient la class disponible 
document.querySelector('.disponible').appendChild(dis)
